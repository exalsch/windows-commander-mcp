    ...
    public object ListTools()
    {
        return new
        {
            tools = new[]
            {
                ...
                Tool("read_ui_tree", "Reads the UI Automation tree for a window. Returns a result object with the flattened 'elements' list plus 'truncated' (true when elements deeper than max_depth were dropped), 'maxDepth', and 'elementCount'. Use control_types / interactable_only to shrink the payload to the elements you care about.",
                    ...
                    StrArray("control_types", "Optional: keep only elements whose UI Automation control type is in this list (e.g. [\"Button\",\"Edit\",\"MenuItem\"]). Omit to return every control type. Truncation is still computed over the full tree."),
                    Bool("interactable_only", "Optional: keep only elements that expose an actionable pattern (Invoke, Toggle, SelectionItem, ExpandCollapse or Value), dropping structural noise like panes and static text.")),
    ...
    private object DispatchUiAutomationTool(string name, JsonElement? arguments, CancellationToken cancellationToken)
    {
        return name switch
        {
            "read_ui_tree" => uiAutomationService.ReadUiTree(
                ...
                GetStringArray(arguments, "control_types"),
                GetBool(arguments, "interactable_only") ?? false),
